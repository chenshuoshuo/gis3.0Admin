...
export default function treeToArray(data, expandAll, parent, level, item) {
  ...
  Array.from(data).forEach(function(record) {
    ...
    tmp.push(record)
    if (record.childrenMapPointTypeList && record.childrenMapPointTypeList.length > 0) {
      const children = treeToArray(record.childrenMapPointTypeList, expandAll, record, _level, item)
